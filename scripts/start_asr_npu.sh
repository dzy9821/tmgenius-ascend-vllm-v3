...
# 3x 1.7B (离线) + 6x 0.6B (在线)，共 9 个 vLLM 实例
# 内存: 1.7B=0.15, 0.6B=0.08, 总计 0.93
...
# ── 在线 0.6B #1~#6 (端口 15008, 15010, 15012, 15014, 15016, 15018) ──
for i in 1 2 3 4 5 6; do
    port=$((15006 + i * 2))  # 15008, 15010, 15012, 15014, 15016, 15018
    echo "=== 启动 Qwen3-ASR-0.6B #${i} (端口 ${port}, mem 0.08) ==="
    ASCEND_RT_VISIBLE_DEVICES=2 \
...
for i in $(seq 1 120); do
    ...
    if [ "$ALL_OK" = "1" ]; then
        ...
        echo "在线 0.6B #4  → http://localhost:15014"
        echo "在线 0.6B #5  → http://localhost:15016"
        echo "在线 0.6B #6  → http://localhost:15018"
        wait
        exit 0
    fi
    sleep 5
