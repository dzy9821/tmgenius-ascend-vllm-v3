...
# ==========================================================
#  1. 启动 2 个离线 ASR 实例 Qwen3-ASR-1.7B (端口 15002, 15004)
# ==========================================================
echo "=== [entrypoint] 启动 Qwen3-ASR-1.7B #1 (离线, 端口 15002, mem 0.21) ==="
ASCEND_RT_VISIBLE_DEVICES=2 \
  ...
  --port 15002 &
VLLM_PIDS+=($!)
echo "[entrypoint] 离线 #1 PID=${VLLM_PIDS[-1]}"

sleep 120
echo "=== [entrypoint] 启动 Qwen3-ASR-1.7B #2 (离线, 端口 15004, mem 0.21) ==="
ASCEND_RT_VISIBLE_DEVICES=2 \
  vllm serve "/weights/Qwen3-ASR-1.7B" \
  --served-model-name Qwen3-ASR-1.7B \
  --gpu-memory-utilization 0.11 \
  ...
  --port 15004 &
VLLM_PIDS+=($!)
echo "[entrypoint] 离线 #2 PID=${VLLM_PIDS[-1]}"

sleep 120

# ==========================================================
#  2. 启动 6 个在线 ASR 实例 Qwen3-ASR-1.7B (端口 15006-15016, 步进 2)
# ==========================================================
echo "=== [entrypoint] 启动 6 个在线 ASR 实例 Qwen3-ASR-1.7B ==="
for i in $(seq 0 5); do
    PORT=$((15006 + i * 2))
    echo "[entrypoint] Qwen3-ASR-1.7B 在线 #$((i+1)) → 端口 $PORT"
    ASCEND_RT_VISIBLE_DEVICES=2 \
      vllm serve "/weights/Qwen3-ASR-1.7B" \
      --served-model-name Qwen3-ASR-1.7B \
      --gpu-memory-utilization 0.11 \
      ...
      --port $PORT &
    VLLM_PIDS+=($!)
    echo "[entrypoint] 在线 #$((i+1)) PID=${VLLM_PIDS[-1]}"
    if [ $i -lt 5 ]; then
        sleep 120
    fi
done
...
for attempt in $(seq 1 120); do
    ...
    if $ALL_OK; then
        ...
        echo "[entrypoint] 离线 #1 → http://localhost:15002"
        echo "[entrypoint] 离线 #2 → http://localhost:15004"
        for i in $(seq 0 5); do
            PORT=$((15006 + i * 2))
            echo "[entrypoint] 在线 #$((i+1)) → http://localhost:$PORT"
        done
        break
    fi
