...
# ==========================================================
#  1. 启动 1 个 Qwen3-ASR-1.7B (端口 15002)
# ==========================================================
echo "=== [entrypoint] 启动 Qwen3-ASR-1.7B (端口 15002, mem 0.13) ==="
...
#  2. 等待 vLLM 端口就绪
# ==========================================================
echo "[entrypoint] 等待 vLLM 服务就绪..."
for attempt in $(seq 1 120); do
    CODE=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 2 http://127.0.0.1:15002/v1/models 2>/dev/null || echo "000")
    if [ "$CODE" = "200" ]; then
        echo "[entrypoint] === vLLM 服务已就绪 → http://localhost:15002 ==="
        break
    fi
...
if [ "$CODE" != "200" ]; then
    echo "[entrypoint] WARNING: vLLM 服务未就绪，继续启动主程序"
fi

# ==========================================================
#  3. 启动主程序（exec 接管进程）
# ==========================================================
export OFFLINE_API_BASES="http://127.0.0.1:15002/v1"
